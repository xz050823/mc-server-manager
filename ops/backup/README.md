...
`2026-07-17` 已用本地冷备份
`hello-new-generation_cold_2026-07-17_01-26-36_+0800.tar.zst` 完成一次非覆盖式生产回档：归档先完成
SHA-256 与 tar 结构校验，再解压到独立暂存目录；停服后把当时的正式目录改名保留为
`/data/minecraft/hello-new-generation.before-rollback-20260717-114542`，最后才切换恢复目录。服务启动、
RCON、玩家登录和实际游玩均正常。验收期间 timer 保持停用，确认无误后通过
...
恢复备份必须另行安排停服维护窗口。本地冷备份的非覆盖式回档已经在生产环境验证：先暂停 timer，
校验 SHA-256 和 tar 结构，解压到独立暂存目录并同步当前 RCON 配置；确认 0 人后停止服务，把当前正式
目录改名保留，再通过同一文件系统内的目录改名切换暂存目录。启动失败时应立即把失败目录移开并把
原目录换回。服务、RCON 和
...
首次生产冷备份、恢复启动、本地归档非覆盖式回档、分卷断点续传和自动 timer 已通过；以下边界已有
